...
class CRVEntry:
    ...
    def to_dict(self):
        chk_count = len(self.chk) if isinstance(self.chk, dict) else 0
        path_count = len(self.PATH) if isinstance(self.PATH, list) else 0
        sk_count = len(self.SK) if isinstance(self.SK, dict) else 0
        return {
            "r_share_bytes": len(self.R),
            "chk_entries": chk_count,
...
class WinternitzThresholdHBSScheme(DistributedThresholdHBSScheme):
    ...
    def recover_winternitz_public_key(self, leaf_index, randomizer, message, revealed):
        ...
        for i in range(self.num_chains):
            pub.append(self.hash_iter(revealed[i], self.w - 1 - digits[i]))

        return WinternitzPublicKey(pub)
    ...
    def verify(self, signature, message=None, public_bundle=None):
        ...
        recovered_public_key = self.recover_winternitz_public_key(
            signature.leaf_index,
            signature.randomizer,
            message,
            signature.revealed,
        ...
        if recovered_public_key is None:
            return False

        return self.verify_merkle_path(
            recovered_public_key.leaf_hash(self),
            signature.auth_path,
            public_bundle.merkle_root,
        )

...
- `benchmarks.py`: Evaluates performance metrics under different parameter settings and saves results to JSON
- `plot_benchmarks.py`: Generates plots based on benchmark results
- `benchmark_results.json`: Stores benchmark outputs for analysis

## Requirements
- Python 3.x
- Standard libraries only: `hashlib`, `secrets`, `statistics`, `time`, `itertools`

Additional libraries (for plotting only):
- `matplotlib`
- `numpy`
...
This will generate:
- `plot_setup_comparison.png`
- `plot_sign_comparison.png`
- `plot_verify_comparison.png`
- `plot_treeheight_Minimal.png`
- `plot_treeheight_Extension1_KOfN.png`
- `plot_treeheight_Extension2_Distributed.png`
- `plot_treeheight_Extension5_Winternitz.png`
- `plot_batch_efficiency.png`
...
Additional observations from plotting:
- Setup time increases with tree height across all schemes.
- Distributed schemes have higher signing latency compared to the baseline due to multi-round coordination.
- Batched schemes significantly reduce amortised signing cost per message.
- Hierarchical batching further improves scalability when batch sizes increase.
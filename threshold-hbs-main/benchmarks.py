...
def signature_size_bytes(sig):
    ...
    # Winternitz Threshold Signature
    if hasattr(sig, "revealed") and hasattr(sig, "randomizer") and hasattr(sig, "auth_path"):
        total = 0
        total += len(sig.randomizer)
        total += sum(len(x) for x in sig.revealed)
        total += merkle_path_size(sig.auth_path)
...
def public_root_size_bytes(bundle):
    return len(bundle.merkle_root)
...
def run_minimal(parties, tree_height, rounds):
    ...
    for i in range(rounds):
        ...
        bundle_sizes.append(public_root_size_bytes(scheme.public_bundle))
    ...
    return {
        ...
        "public_root_size_bytes": round(statistics.mean(bundle_sizes), 2),
        "crv_size_bytes": 0,
        "round1_comm_bytes": 0,
        "round2_comm_bytes": 0,
        "total_comm_bytes": 0,
